#define DC_Calib_Comp_SHMS_cxx

#include "DC_Calib_Comp_SHMS.h"
#include <TStyle.h>
#include <TCanvas.h>
#include <TLine.h>
#include <TMath.h>
...
void DC_Calib_Comp_SHMS::Begin(TTree * /*tree*/)
{  
  printf("\n\n");
  TString option = GetOption();
}

void DC_Calib_Comp_SHMS::SlaveBegin(TTree * /*tree*/)
{
  printf("\n\n");
  TString option = GetOption();
...
Bool_t DC_Calib_Comp_SHMS::Process(Long64_t entry)
{
  fReader.SetEntry(entry);

  if (P_hgcer_npeSum[0] < 1.0) return kTRUE;
  if (T_coin_pEL_CLEAN_tdcTime[0] < 0) return kTRUE;
  for (Int_t i = 0; i < 12; i++){
...
void DC_Calib_Comp_SHMS::SlaveTerminate()
{
}

void DC_Calib_Comp_SHMS::Terminate()
{
  TString option = GetOption();
  ...
  TFile *Histogram_file = new TFile(Form("HISTOGRAMS/SHMS_DCCalib_Comp_Run%i.root", Run),"RECREATE");
