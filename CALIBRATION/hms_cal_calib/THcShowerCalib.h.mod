...
class THcShowerCalib {
 ...
 public:
  ...
  void SaveAlphas();
  void SaveRawData();
  // 29/04/21 - SJDK - Added in new variables/fns from Bill Henry
  void FillCutBranch();
  void FillHitsGains();
  ...
  // 29/04/21 - SJDK - Added in new histos from Bill Henry
  TH1F* hEunc;
  TH1F* hEuncSel;
  TH1F* hEcal;
  TH1F* hEcalNoCor;
...
void THcShowerCalib::ReadThresholds() {
  ...
  // 29/04/21- SJDK - Bill Henry added in new parameter to get
  getline(fin, line);  iss.str(line);
  iss >> sigmaRange;
...
void THcShowerCalib::Init() {
 ...
 cout << "THcShowerCalib::Init: Root file name = " << fname << endl;
  ...
  // 29/04/21 - SJDK - Bill Henry incorporated some new branches and switched some others, modified
  ...
  // 29/04/21 - SJDK - Bill Henry incorporated some new histograms
...
void THcShowerCalib::CalcThresholds() {
  ...
  // 29/04/21 - SJDK - Changes to fitting routine from Bill Henry incorporated
...
void THcShowerCalib::FillHEcal() {
  ...
  for (UInt_t ientry=fNstart; ientry<fNstop; ientry++) {
    ...
    if (ReadShRawTrack(trk, ientry)) {
      //    trk.Print(cout);
      // 29/04/21 - SJDK - Changes from Bill Henry
      //************wph*************
...
// 29/04/21 - SJDK - New functions from Bill Henry
//------------------------------------------------------------------------------
