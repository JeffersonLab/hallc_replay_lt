...
void hcal_calib(string Prefix, int RunNumber, int nstop=-1, int nstart=0) {
 ...
 // Save canvas in a pdf format.
  TString TOutpdf = Form("%s_%d_%d.pdf",Prefix.c_str(),RunNumber,nstop);
  Canvas->Print(TOutpdf+'(');
...
//wph
 ...
 pt->AddText("Cherenkov");
 pt->AddText("Beta");
 pt->AddText("Delta");
 pt->AddText("nClusters==1");
 pt->AddText("nTracks==1");
 pt->Draw();
 ...
 // Save canvases
 Canvas2->Print(TOutpdf);
 Canvas3->Print(TOutpdf+')');
 }
