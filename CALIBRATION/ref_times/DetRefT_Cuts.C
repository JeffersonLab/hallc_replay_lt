...
void DetRefT_Cuts::SlaveBegin(TTree * /*tree*/)
{
  ...
  h1HCerRefTime = new TH1F("HCerRefTime", "HMS Cherenkov RefTime Dist; RefTime/ns", 500, 0, 500);
  h1PHGCRefTime = new TH1F("PHGCRefTime", "SHMS HGCv RefTime Dist; RefTime/ns", 500, 0, 500);
  GetOutputList()->Add(h1HCerRefTime);
  for (Int_t i = 0; i < 12; i++){ 
    h1HDCRefTime[i] = new TH1F(Form("HDC%s_RefTime", dc_pl_names[i].c_str()), Form("HMS DC Plane %s Ref Time; RefTime/ns", dc_pl_names[i].c_str()), 500, 0, 500);
    h1PDCRefTime[i] = new TH1F(Form("PDC%s_RefTime", dc_pl_names[i].c_str()), Form("SHMS DC Plane %s Ref Time; RefTime/ns", dc_pl_names[i].c_str()), 500, 0, 500);
  ...
  for (Int_t i = 0; i < 4; i++){ // Loop over planes
	...
	h1HHodRefTime[i][j][k] = new TH1F(Form("HHod%s%s%s_RefTime", hod_pl_names[i].c_str(), hod_side[j].c_str(), hod_digi[k].c_str()), Form("HMS Hodo Plane %s %s %s Ref Time; RefTime/ns", hod_pl_names[i].c_str(), hod_side[j].c_str(), hod_digi[k].c_str()), 500, 0, 500);
	h1PHodRefTime[i][j][k] = new TH1F(Form("PHod%s%s%s_RefTime", hod_pl_names[i].c_str(), hod_side[j].c_str(), hod_digi[k].c_str()), Form("SHMS Hodo Plane %s %s %s Ref Time; RefTime/ns", hod_pl_names[i].c_str(), hod_side[j].c_str(), hod_digi[k].c_str()), 500, 0, 500);
...
void DetRefT_Cuts::Terminate()
{
  ...
  // for (Int_t i = 0; i < 12; i++){ 
  //   h1HDCRefTime->Write(Form("HMS DC %s RefTime", dc_pl_names[i].c_str()));
  // }
  // for (Int_t i = 0; i < 4; i++){ // Loop over planes
  //   for (Int_t j = 0; j < 2; j++){ // Loop over ends
  //     for (Int_t k = 0; k < 2; k++){ // Loop over digitiser
  // 	h1HHodRefTime[i][j][k]->Write(Form("HMS Hodo %s %s %s RefTime", hod_pl_names[i].c_str(), hod_side[j].c_str(), hod_digi[k].c_str()));
  //     }
  //   }
  // }

  // TDirectory *DSHMS = Histogram_file->mkdir("SHMS Ref Time Histos"); DSHMS->cd();
  // h1PHGCRefTime->Write("SHMS Cherenkov RefTime");
  // for (Int_t i = 0; i < 12; i++){ 
  //   h1PDCRefTime->Write(Form("SHMS DC %s RefTime", dc_pl_names[i].c_str()));
  // }
  // for (Int_t i = 0; i < 4; i++){ // Loop over planes
  //   for (Int_t j = 0; j < 2; j++){ // Loop over ends
  //     for (Int_t k = 0; k < 2; k++){ // Loop over digitiser
  // 	h1PHodRefTime[i][j][k]->Write(Form("SHMS Hodo %s %s %s RefTime", hod_pl_names[i].c_str(), hod_side[j].c_str(), hod_digi[k].c_str()));
  //     }
  //   }
  // }
