...
void DetTCuts_Coin::SlaveBegin(TTree * /*tree*/)
{
  ...
  for (Int_t npl = 0; npl < hod_planes; npl++){ // Loop over all hodoscope planes
	...
	h2pHodoTDiffADCAmp[npl][nside][ipmt] = new TH2F(Form("pHodo%s%d%s_tDiffADCAmp", hod_pl_names[npl].c_str(),ipmt+1,nsign[nside].c_str() ), Form("SHMS Hodo %s%d%s AdcTdcTimeDiff vs ADC Pulse Amp; Time (ns); Charge (pC)", hod_pl_names[npl].c_str(), ipmt+1,nsign[nside].c_str()), 200, -100, 100, 500, 0, 500);
