...
Bool_t fileNameMatch(const char *fileName){
	TString fileNameString = fileName;
	Int_t startIndex = 0;
	TPMERegexp regexp("Pion_coin_replay_calibration_(\\d*)_-1.root", "g");
...
int run(const char *fileName){
	...
	// Cut branchs, temp study
	TBranch* branchX = tree->GetBranch("P.aero.xAtAero");
	TBranch* branchY = tree->GetBranch("P.aero.yAtAero");
	TBranch* branchHGC = tree->GetBranch("P.hgcer.npeSum");
	...
	Double_t y = 0;
	Double_t HGCnpe = 0;
	branchX->SetAddress(&x);
	branchY->SetAddress(&y);
	branchHGC->SetAddress(&HGCnpe);
	branchX->SetAutoDelete(kTRUE);
	branchY->SetAutoDelete(kTRUE);
	branchHGC->SetAutoDelete(kTRUE);
