...
void pcal_calib(string Prefix,int RunNumber, int nstop=-1, int nstart=0) {
  ...
  // 29/04/21 - SJDK - Added from Bill Henry updates
  ...
  // 29/04/21 - SJDK - Added in new plots from Bill Henry
  //************wph add plots****************
  ...
  // Save the new plots too! Do them all in one pdf!
  TString foutpdf = Form("%s_%d_%d.pdf",Prefix.c_str(),RunNumber,nstop);
  Canvas->Print(foutpdf+"(");
  Canvas2->Print(foutpdf);
  Canvas3->Print(foutpdf+")");
