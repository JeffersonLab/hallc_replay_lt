void FullReplay_KaonLT_HeeP_Sing_HMS (Int_t RunNumber = 0, Int_t MaxEvent = 0) {
  ...
  const char* ROOTFileNamePattern = "ROOTfiles/Analysis/HeeP/Kaon_HMS_replay_production_%d_%d.root";
  ...
  if (RunNumber >= 4965 && RunNumber <= 5334){
   analyzer->SetCutFile("DEF-files/PRODUCTION/KaonLT_DEF/Aero_1p011/Offline_HeeP_Sing_HMS_Cuts.def");
  }
  else if (RunNumber >= 7940 && RunNumber <= 8356){
   analyzer->SetCutFile("DEF-files/PRODUCTION/KaonLT_DEF/Aero_1p011/Offline_HeeP_Sing_HMS_Cuts.def");
  }
  else {
   analyzer->SetCutFile("DEF-files/PRODUCTION/KaonLT_DEF/Offline_HeeP_Sing_HMS_Cuts.def");
  }
  ...
  Form("REPORT_OUTPUT/Analysis/General/replay_hms_heep_%d_%d.report", RunNumber, MaxEvent));  // optional
