void replay_production_coin_Lumi (Int_t RunNumber = 0, Int_t MaxEvent = 0) {
 ...
 // Add Physics Module to calculate secondary (scattered hadrons) beam kinematics
  ...
  // Define DEF-file+
  analyzer->SetOdefFile("UTIL_PROTON/DEF-files/luminosity_coin_production.def");
  // Define cuts file
  analyzer->SetCutFile("UTIL_PROTON/DEF-files/luminosity_coin_production_cuts.def");  // optional
  // File to record accounting information for cuts
  analyzer->SetSummaryFile(Form("UTIL_PROTON/REPORT_OUTPUT/COIN/PRODUCTION/summary_production_%d_%d.report", RunNumber, MaxEvent));  // optional
